...
import { joinCollection } from "@/lib/actions";
...
export default function JoinCollection() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await joinCollection(formData);
    ...
    if (result.error) {
      ...
      router.push(`/collections/${result.collectionId}?auth=${encodeURIComponent(formData.get("password") as string)}`);
    }
  }
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center py-20 px-6">
        ...
        <div className="w-full max-w-md animate-fade-in-up">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Join Collection</h1>
            <p className="text-slate-400">Enter collection details to view or submit slides</p>
          </div>
          ...
          <div className="glass-effect rounded-2xl p-8 border border-indigo-500/20">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="collectionId" className="block text-sm font-medium text-slate-300 mb-2">
                  Collection ID
                </label>
                <input
                  type="text"
